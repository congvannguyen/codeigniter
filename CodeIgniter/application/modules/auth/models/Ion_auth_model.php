...
class Ion_auth_model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->username = $this->config->item('identity_username', 'ion_auth');
    ...
    public function count() {
        return $this->db->select()->get($this->tables['users'])->num_rows();
    }

    public function getRange($perpage, $offset) {
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        //congnv
        if ($this->username_check($additional_data['username'])) {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        if (!filter_var($identity, FILTER_VALIDATE_EMAIL)) {
            $query = $this->db->select($this->username . ', email, id, password, active, last_login')
                    ->where($this->username, $identity)
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['users']);
        } else {
